...
authenticate the connection with Coder. If you do not have a certificate
configured, or if there is an issue with the certificate itself, you will
receive this error.
...
- If the local registry has not been created, and the self-signed cert has not
  been generated, [please see our
...
- Check to see if your `registry.crt` file is stored in the correct location on
  ...
  container runtime, it may be in one of the following places:
...
- Ensure that the self-signed certificate secret has been created in your
  Kubernetes cluster:
