...
- web: fixed issue where JetBrains 2022.1 IDE versions would not open in
  Projector.
- web: fixed issue with EC2 provider workspaces hanging on build
  if a filesystem issue prevented mounting user volumes.
...
- infra: on new installations with **Self-contained workspace builds** enabled,
  workspaces using images without `curl` available (for example, `ubuntu:latest`)
  will fail to build. **Workaround:** use a base image with `curl` available.
  For example: `codercom/enterprise-base:ubuntu`
