...
- web: The root dashboard PWA manifest has been removed. Please uninstall the Coder PWA.
  Coder is moving forward with a PWA-per-application approach. To install `code-server PWA`:
  ...
  - **Note:** Currently, the PWA scope is limited *per environment*. This means each environment
  requires that the `Code` application is installed as a PWA. We recognize that this is not an
  ideal workflow and as such, it will be addressed in a future iteration.
