...
- web: Added ability to choose the SSH keygen algorithm Coder uses when
  generating SSH keys.
- infra: Added connection cache based on open DataChannels.
- infra: Added option to opt-in to OIDC's new refresh feature, allowing
...
- infra: Added `TRUST_PROXY_IP` Coder environment variable.
- web: Added ability to enable/disable SSH access to the edit workspace
  providers page.
- infra: Added functionality so Coder agent output is written to logs.
- web: Images tags now update asynchronously.
- infra: Added WebRTC service (including ICE server routes)
- web: Removed appProtocol from `coderd`.
...
- web: Fixed issue with envproxy routes to resolve broken PWA bookmarks.
...
- infra: Fixed OIDC configuration vulnerability.
