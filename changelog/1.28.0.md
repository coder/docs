...
- web: added support for requesting additional scopes from the OIDC authentication
  provider.
...
- web: fixed issue with Coder not persisting custom resource allocation
  requests.
- web: fixed issue where existing OIDC users cannot log in when the
  license is at maximum usage.
